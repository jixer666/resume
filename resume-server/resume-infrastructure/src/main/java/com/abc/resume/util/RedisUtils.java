...
public class RedisUtils {
     ...
     *
     * @param key 键 不能为null
     * @return 时间(秒) 返回0代表为永久有效，-2代表不存在key
     */
    ...
    /**
     * 缓存更新：仅当 key 已存在时更新缓存值，并保留原有过期时间
     *
     * @param key   键
     * @param value 新值
     * @return true更新成功，key不存在或异常返回false
     */
    public static Boolean update(String key, Object value) {
        try {
            ...
            if (expire != null && expire > 0) {
                stringRedisTemplate.opsForValue().set(key, objToStr(value), expire, TimeUnit.SECONDS);
            } else {
                stringRedisTemplate.opsForValue().set(key, objToStr(value));
            }
            return true;
     ...
     * @return Map列表，每个Map对应一个Hash键的全部键值对
     */
    public static List<Map<Object, Object>> hmgetByPrefix(String pattern) {
        ...
        try {
            ...
            for (Object result : results) {
                resultList.add((Map<Object, Object>) result);
            }
        } finally {
            if (cursor != null) {
                cursor.close();
    ...
    public static Boolean hmset(String key, Map<String, Object> map) {
        // 统一处理将value转为string
        Map<String, String> converted = new HashMap<>(map.size() * 2);
        map.forEach((k, v) -> {
            if (v != null) {
        ...
        try {
            stringRedisTemplate.opsForHash().putAll(key, converted);
            return true;
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return false;
